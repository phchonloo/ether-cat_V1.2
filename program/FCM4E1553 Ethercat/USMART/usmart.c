...
//	}
//	usmart_dev.runtime+=TIM_GetCnt(TIM4);
...
//{ 		    		  			    
//	if(TIM_GetIntStatus(TIM4,TIM_INT_UPDATE)==SET)//溢出中断
//	{	
//		usmart_dev.scan();	//执行usmart扫描	
//		TIM_SetCnt(TIM4,0);		//清空定时器的CNT
//		TIM_SetAutoReload(TIM4,100);//恢复原来的设置		    				   				     	    	
//	}				   
//	TIM_ClrIntPendingBit(TIM4,TIM_INT_UPDATE);  //清除中断标志位    
//}
////使能定时器4,使能中断.
...
//	RCC_EnableAPB1PeriphClk(RCC_APB1_PERIPH_TIM4,ENABLE);  ///使能TIM4时钟
...
//	
//	TIM_InitTimeBase(TIM4,&TIM_TimeBaseInitStructure);//初始化定时器4
//	
//	TIM_ConfigInt(TIM4,TIM_INT_UPDATE,ENABLE); //允许定时器4更新中断
//	TIM_Enable(TIM4,ENABLE); //使能定时器4
// 
