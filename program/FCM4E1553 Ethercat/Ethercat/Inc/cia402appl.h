...
/*
 * 0x6040控制字常用启动顺序：
 *   0x0006：Shutdown，准备上电，目标状态为Ready To Switch On。
 *   0x0007：Switch On，目标状态为Switched On。
 *   0x000F：Enable Operation，目标状态为Operation Enabled。
 *
 * 注意：0x0007在Operation Enabled状态表示Disable Operation，
 *      会让轴退出运行状态；命令含义必须结合当前状态判断。
 */
...
#define STATUSWORD_HOMING_ERROR                              0x2000 /**< \brief Homing error (used in homing mode)*/
...
-----------------------------------------------*/
/*
 * 主站判断0x6041状态时应先执行：status_word & 0x006F。
 * 启动过程中常见结果：
 *   0x0021：Ready To Switch On，控制字0x0006已处理。
 *   0x0023：Switched On，控制字0x0007已处理。
 *   0x0027：Operation Enabled，控制字0x000F已处理。
 * 完整状态字还可能包含0x0200 Remote、0x0010 Voltage Enabled等位。
 */
...
-----------------------------------------------*/
/*
 * 以下数值只供MCU内部变量i16State使用，不是主站读取的0x6041值。
 * 例如内部STATE_OPERATION_ENABLED为0x0010，主站状态值为0x0027。
 */
...
#define PROFILE_VELOCITY_MODE       3 /**< \brief Velocity Profile mode*/
#define PROFILE_VELOCITY_MOCE       PROFILE_VELOCITY_MODE /**< \brief Backward-compatible alias*/
...
#define MAX_AXES    1 /**< \brief This carrier drives one physical motor axis*/
...
typedef __packed struct STRUCT_PACKED_START
{
...
typedef __packed struct STRUCT_PACKED_START
{
    ...
    INT32 ObjPositionActualValue; /**< \brief Actual position (0x6064)*/
    INT32 ObjVelocityActualValue; /**< \brief Actual velocity (0x606C)*/
    INT16 ObjModesOfOperationDisplay; /**< \brief Mode display plus 8bit padding*/
}STRUCT_PACKED_END
TCiA402PDO1A02;
 ...
 *
 * SubIndex 0 : 1<br>
 * SubIndex 1 : combined csp/csv module is set by default<br>
 * SubIndex 2 : 0
 */
...
{5, {0x60410010,0x60640020,0x606C0020,0x60610008,0x00000008}},/*TOBJ1A02*/
...
CYCLIC_SYNC_VELOCITY_MODE,/*(INT16) ModeOfOperation 0x6060*/
CYCLIC_SYNC_VELOCITY_MODE,/*(INT16) Mode Of Operation Display 0x6061*/
...
= {
   ...
   /* Object 0x1A02 */
   {NULL,NULL, 0x1A02, {DEFTYPE_PDOMAPPING, 5 | (OBJCODE_REC << 8)}, asEntryDesc0x1A02, aName0x1A02, NULL, NULL, NULL, 0x0000 },	
   /* Object 0x603F */
